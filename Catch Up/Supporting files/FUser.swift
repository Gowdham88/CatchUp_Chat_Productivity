...
class FUser {
    ...
    class func registerUserWith(email: String, password: String, firstName: String, lastName: String, avatar: String = "", completion: @escaping (_ error: Error?) -> Void ) {
        ...
        Auth.auth().createUser(withEmail: email, password: password, completion: { (firuser, error) in
            ...
            saveUserToDatabase(fUser: fUser)
            completion(error)
...
func saveUserToDatabase(fUser: FUser) {
    
    reference(.User).child(fUser.objectId).setValue(userDictionaryFrom(user: fUser) as! [String : Any]) { (error, reference) in
        
        print("reference is \(reference) and error \(error?.localizedDescription) ")
    }
...
//New firestoreFIRDatabaseReference
...
//    }
    ...
    reference(.User).child(userId).observe(DataEventType.value, with: { (snapshot) in
        
        guard let snap = snapshot as DataSnapshot? else { return }
        
        if snap.exists() {
            
            print("updated current users param")
            
            UserDefaults.standard.setValue(snap.value as! NSDictionary, forKeyPath: kCURRENTUSER)
            UserDefaults.standard.synchronize()
        }
...
//    }
    ...
    reference(.User).child(userId).observeSingleEvent(of: DataEventType.value) { (snapshot, success) in
        
        guard let snap = snapshot as DataSnapshot? else { return }
        
        if snap.exists() {
            
            let user = FUser(_dictionary: snap.value! as! NSDictionary)
            completion(user)
        }else {
...
func getUsersFromDatabase(withIds: [String], completion: @escaping (_ usersArray: [FUser]) -> Void) {
...
//        }
        ...
        reference(.User).child(userId).observeSingleEvent(of: DataEventType.value) { (snapshot, err) in
...
func updateCurrentUserInDatabase(withValues : [String : Any], completion: @escaping (_ error: Error?) -> Void) {
...
//        }
        ...
        reference(.User).child(currentUserId).updateChildValues(withValues) { (error, ref) in
...
//
//    updateCurrentUserInDatabase(withValues: [kPUSHID : newId]) { (error) in
//        if error != nil {
